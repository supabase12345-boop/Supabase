// ===================================
// supabase.js - Elite Capital (النسخة النهائية المستقرة)
// ===================================
...
function initSupabase() {
    ...
    try {
        ...
        supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
            auth: {
                persistSession: false, // تعطيل جلستها الداخلية
                autoRefreshToken: false,
                storageKey: 'elite_capital'
            }
        });
...
async function registerUser(userData) {
    try {
        ...
        if (userData.referralCode) {
            ...
            if (referrer) referredBy = userData.referralCode;
        }
...
async function createWithdrawal(withdrawalData) {
    try {
        ...
        if (user.balance < totalAmount) throw new Error('الرصيد غير كافي');
    ...
    try {
        ...
        return {
            ...
            data: {
                ...
                totalDeposits: users.reduce((sum, u) => sum + (u.total_earned || 0), 0),
...
// ========== الدردشة الجماعية ==========
async function getGroupChatMessages(limit = 100) {
    try {
        const { data, error } = await supabaseClient
...
async function sendChatMessage(chatId, userId, message) {
    try {
        if (!message?.trim()) throw new Error('الرسالة لا يمكن أن تكون فارغة');
