...
async function getGroupChatMessages(limit = 50) {
    try {
        console.log('محاولة جلب الرسائل...');
        const { data, error } = await supabaseClient
...
async function createGroupChat(userId, message, imageFile = null) {
    try {
        // التحقق من وجود اشتراك نشط أو المستخدم Admin
        const { data: user, error: userError } = await supabaseClient
            .from('users')
            .select('has_active_subscription, is_admin')
            .eq('id', userId)
            .single();

        if (userError) throw userError;
        
        if (!user.has_active_subscription && !user.is_admin) {
